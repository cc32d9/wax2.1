...
namespace eosio { namespace chain {
   ...
   namespace {
      ...
      constexpr int offset_to_block_start(uint32_t version) { 
         return version >= 4 ? sizeof(uint32_t) + 1 : 0;
      }
      ...
      void unpack(Stream& ds, log_entry_v4& entry){
         ...
         uint64_t current_stream_offset = ds.tellp() - start_pos;

         int64_t bytes_to_skip = static_cast<int64_t>(entry.size) - sizeof(uint64_t) - current_stream_offset;
...
FC_REFLECT_DERIVED(eosio::chain::log_entry_v4, (eosio::chain::signed_block), (compression)(offset) )
...
namespace {
   ...
   class block_log_data {
    ...
    public:
      ...
      void light_validate_block_entry_at(uint64_t pos, uint32_t expected_block_num) const {
         ...
         if (version() >= 4) {
            uint32_t offset = read_buffer<uint32_t>(data()+pos);
            uint64_t position_in_entry = read_buffer<uint64_t>(data() + pos + offset - sizeof(uint64_t));
            EOS_ASSERT(pos == position_in_entry, block_log_exception, 
      ...
      full_validate_block_entry(fc::datastream<const char*>& ds, const block_id_type& previous_block_id, log_entry& entry) {
   ...
   detail::block_log_impl::block_log_impl(const fc::path& data_dir) {
      ...
      auto log_size   = fc::file_size(block_file.get_file_path());
      auto index_size = fc::file_size(index_file.get_file_path());
   ...
   void block_log::construct_index(const fc::path& block_file_name, const fc::path& index_file_name) {
      ...
      ilog("first block= ${first}         last block= ${last}",
           ("first", log_data.first_block_num())("last", (log_data.first_block_num() + num_blocks)));
   ...
   fc::path block_log::repair_log(const fc::path& data_dir, uint32_t truncate_at_block) {
      ...
      auto block_file_name = block_log_path.generic_string();
      ...
      try {
         try {
            while (ds.remaining() > 0 && block_num < truncate_at_block) {
               std::tie(block_num, previous) = block_log_data::full_validate_block_entry(ds, previous, entry);
               if (block_num % 1000 == 0)
