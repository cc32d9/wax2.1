  ...
  - label: ":darwin: macOS 10.15 - Build"
    ...
    plugins:
      - EOSIO/anka#v0.6.0:
          ...
          vm-registry-tag: clean::cicd::git-ssh::nas::brew::buildkite-agent::eos-macos-10.15-pinned-18ea423dbfc164aa32ad8e16e008fb8dc5751356
          modify-cpu: 12
          modify-ram: 24
          always-pull: true
          debug: true
    ...
    env:
      ...
      REPO_COMMIT: 96a72ff63f53db82c80a177de50829e824417238
      TEMPLATE: 10.15.1_6C_14G_40G
      ...
      PROJECT_TAG: eos-macos-10.15-pinned-18ea423dbfc164aa32ad8e16e008fb8dc5751356
    timeout: 180
    agents: "queue=mac-anka-large-node-fleet"
...
cat <<EOF
  - label: ":darwin: macOS 10.15 - nodeos_forked_chain_lr_test"
    command:
      - "git clone $BUILDKITE_REPO eos && cd eos &&  git checkout -f $BUILDKITE_COMMIT && git submodule update --init --recursive"
      - "cd eos && buildkite-agent artifact download build.tar.gz . --step ':darwin: macOS 10.15 - Build' --build $BUILDKITE_TRIGGERED_FROM_BUILD_ID && tar -xzf build.tar.gz"
